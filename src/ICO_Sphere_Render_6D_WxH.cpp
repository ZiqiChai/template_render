...
int img_size_width=0;
int img_size_height=0;
...
void saveImg(string filename, Mat& depth)
{
	...
	if (maxContours.size())
	{
		...
		if (r0.width<img_size_width)
		{
			r0.x -= int((img_size_width-r0.width)/2);
			if (r0.x<0){r0.x=0;}
		}
		if (r0.height<img_size_height)
		{
			r0.y -= int((img_size_height-r0.height)/2);
			if (r0.y<0){r0.y=0;}
		}
		r0.width = img_size_width;
		r0.height = img_size_height;
		Mat rgbImg = img(r0);
		Mat depImg  = depth(r0);
...
int main(int argc,char *argv[])
{
	if (argc<3)
	{
		std::cout<<"please input desired img_size_width[argv[1]] and img_size_height[argv[2]], optional parameter FileName[argv[3]]"<<std::endl;
	}
	sscanf( argv[1],"%d",&img_size_width);
	sscanf( argv[2],"%d",&img_size_height);
	...
	for(r = Config::get<float>("r_start");r<=Config::get<float>("r_end");r+=Config::get<float>("r_delta"))
	{
		...
		{
			// init a new icohedron at diameter r (unit: meter/Scale2millimeter)
			...
			for (int i=0;i<vec_coordinate.size();i++)
			{
				...
				//always convert m to mm for render
				...
				if(xc == 0 && yc == 0)
				{
					...
					if(zc > 0)
						...
						zc = -sqrt(zc*zc - xc*xc - yc*yc);
					cout<<"adjusted -- xc[mm]: "<< xc <<"; yc[mm]: "<< yc <<"; zc[mm]: "<< zc <<"; gamma[deg]: "<<angle_gamma_ratation<<endl;
				}
